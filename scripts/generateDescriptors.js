...
const apiUrl = 'http://api.gbif.org/v1/';
const helpers = require('./helpers');
const _ = require('lodash');

// given a es index, a datasetKey, and a resolution, then generate a bunch of descriptors
async function generateDescriptors(esUrl, datasetKey, resolution, facetSize) {
  ...
  for (var i = 0; i < taxa.length; i++) {
    ...
    let countriesResult = (await axios.get(apiUrl + `occurrence/search?facet=country&limit=0&facetLimit=1&${resolution}=${taxa[i].name}&datasetKey=${datasetKey}`)).data;
    ...
    if (countries.length > 0) {
      let location = helpers.getLocationFromCountryCode(countries[0].name);
      let counts = helpers.getCounts(countries[0].count);
      descriptors.push({
        ...taxon,
        ...location,
        ...counts,
      ...
      let diffCount = taxa[i].count - countries[0].count;
      if (diffCount > 10) {
...
async function createDescriptors() {
  for (var i = 0; i < datasets.length; i++) {
    await generateDescriptors(esUrl, datasets[i].datasetKey, datasets[i].resolution, datasets[i].facetSize)
  }
}
